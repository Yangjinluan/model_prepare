...
##################数据集
    ...
    # 随机抽取子集
    subset_indices = torch.randperm(len(test_dataset))[:subset_size].tolist()
    # print(subset_indices.size())
    # print(subset_indices)
    # subset = test_dataset[:subset_size]

    ## loader 为部分测试集， loader_test为全量测试集
    loader = DataLoader(torch.utils.data.Subset(test_dataset, subset_indices), batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=True)
...
#---------------------Patch-wise Trojan---------------------------
    ...
    for i, (x_p, y_p) in enumerate(loader):
        ...
        # else:
        #     # CLIPVisionModel
        #     # model = cast(CLIPVisionModel, model)
        #     out = model(x_p + delta)[1]
        #     # get logits
        #     logits = ...
